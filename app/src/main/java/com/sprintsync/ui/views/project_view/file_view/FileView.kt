...
import com.sprintsync.utils.FileUtility

@Composable
fun FileView(projectId: String? = null, context: Context) {
            ...
            LazyColumn(modifier = Modifier.animateContentSize()) {
                if (attachmentList != null) {
                    items(attachmentList) { file ->
                        if (file.name.contains(searchTerm)) FileCard(
                            file,
                            onClick = { name, content ->
                                FileUtility.downloadFile(content, name, context)
                            })
                    }
                }
            }
